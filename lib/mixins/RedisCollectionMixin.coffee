...
# должен быть заиспользован модуль для конекта к редису
...
# поискав в интернете информацию по joins для редиса, можно утвердиться в мысли что база этого не поддерживает. (либо решать на коленках на программном уровне.)
# TODO: надо при парсинге запроса поставить эксепшены на тех дерективах, которые не поддерживаются базой данных. и оставить только "получение по id" и "запись/обновление по id"

# так же в модуле (расширении) для работы с редисом надо предусмотреть RedisSerializer - т.к. в редисе все value должны быть строками. т.е. сериалайзер должен перед отдачей результата в приложение делать JSON.parse, а перед записью данных в редис делать JSON.stringify
# так же в модуле (расширении) надо релизовать класс для работы с редисом по технологии pub/sub  - но надо решить какая часть в фреймворке LeanRC будет соответствовать этой технологии.
